...
- v5 - по итогам сплошной ревизии кода против ядра: разрешена ручная отмена живого заказа с правилом возврата стока, лестница ретраев outbox приведена к достижимым ступеням, определён флаг частичной оплаты в админке, зафиксировано, как считается IP клиента за обратным прокси. Детали - §18.

- v6 - витрина получает казахский визуальный язык: палитра снята с ковра, цвета вынесены в токены, орнамент рисуется SVG-компонентом, зафиксированы движение, слоган и типографика. Часть §7.4 про палитру и декор заменена. Детали - §19.
- v7 - к закрытому списку UI-примитивов §7.2 добавлен `Totals`: итоги заказа рендерились тремя копиями разметки в корзине, чекауте и статусе заказа. Детали - §20.
...
## 18. Решения v5, по итогам ревизии кода против ядра

Ревизия 2026-08-01 сверила `main` с ядром целиком. Часть найденного оказалась
не отступлением от контракта, а противоречием внутри самого контракта; часть -
решением, которое реализовано, но которое следующая сессия из ядра не выведет.
Порядок - по разделу, который дополняется.

### 18.1 Отмена живого заказа (§5.1, §4)

Диаграмма §5.1 рисовала `cancelled` только под `expired`, поэтому админка не
могла закрыть заказ, пока тот не протухнет сам. При этом проза той же §5.1
называет `cancelled` статусом, который «снят вручную из админки», а §8.4
перечисляет отмену среди ручных переходов. Разрешено противоречие в пользу
прозы, диаграмма дополняется двумя стрелками:
...
Остальная машина не меняется, `refunded` по-прежнему приходит только из IPN, а
`expired` принадлежит воркеру резервов. `ManualTargets` (§17.5) не расширяется:
`cancelled` в нём был всегда, теперь он достижим.

**Правило стока при отмене** (§4 его не описывал, потому что отмена жила только
после `expired`, где резерв уже снят):

| Откуда отменяют | Что происходит со стоком |
|---|---|
| `awaiting_payment` | `reserved -= qty` - единицы ещё только зарезервированы |
| `paid` | `stock += qty` - единицы уже списаны, но заказ не отгружен |

Обе операции идут в той же транзакции, что и смена статуса и запись в
`notifications`.

### 18.2 Лестница ретраев outbox (§5.5)

§5.5 перечисляла пять задержек при лимите «максимум 5 попыток». Между пятью
попытками четыре промежутка, поэтому пятая задержка (6ч) была недостижима.
Лимит попыток - более явное утверждение, поэтому сохранён он, а перечень
приведён к достижимому:

```
30с, 2м, 10м, 1ч; максимум 5 попыток
```

### 18.3 Флаг частичной оплаты (§5.4)

§5.4 требует при `partially_paid` оставить заказ в `awaiting_payment` и дать
«флаг в админку», но не говорила, что это за флаг. Реализовано так: в списке
заказов строка получает бейдж `part paid` (`ToneWarning`), на карточке заказа -
`Alert` с объяснением, что оплата неполная и заказ ждёт остатка либо ручного
разбора. Источник признака - текущий сырой статус провайдера: в списке считается
в SQL как `EXISTS` по `payments.status`, на карточке берётся из новейшего события
лога, который карточка и так грузит.

### 18.4 Адрес клиента за обратным прокси (§9.6)

§9.6 задаёт лимиты «на IP», но не говорит, откуда этот IP берётся. За Caddy
приложение видит только контейнер прокси, поэтому `RemoteAddr` схлопывал все
лимиты в один общий на весь магазин. Зафиксировано:

- единственный источник адреса для любого решения «на IP» - `middleware.ClientIP`;
- `X-Forwarded-For` читается, **только** если peer - loopback, RFC1918 или ULA,
  то есть наша собственная сеть; запрос напрямую из интернета сохраняет свой
  `RemoteAddr`, что бы он ни прислал в заголовке;
- цепочка обходится справа налево до первой записи, которую мог написать сам
  клиент.

Переменной окружения для списка доверенных прокси намеренно нет: набор env
заморожен (§10.2, `SKELETON.md` §4), а доверие выводится из того, что адрес
принадлежит частной сети. Появится второй прокси или CDN - это `CONTRACT GAP`.

---

## 19. Решения v6: визуальный язык витрины

§7.4 задавала минимализм: белый фон, один акцент, «никаких теней-градиентов».
Витрина получает фирменный казахский облик, поэтому §7.4 в части палитры и
декора **заменяется** этим разделом. Всё, что §7.4 говорит про мобильный
приоритет, брейкпоинты Tailwind, `loading="lazy"` и фиксированный
`aspect-ratio`, остаётся в силе без изменений.

### 19.1 Область действия

Новый облик применяется к витрине (`layout.Shop` и её страницы: главная,
товар, корзина, чекаут, `/order/{token}`, ошибки, kitchen sink). Админка в
браузере (`layout.AdminWeb`) остаётся светлой и служебной: продавец смотрит в
неё таблицы и графики, а не бренд. Mini App (`layout.AdminMini`) по-прежнему
красится из `themeParams` Telegram (§7.5, S7) - от казахского слоя ей достаются
только орнаментальные разделители и золотой акцент, ни один цвет фона или
текста из `themeParams` не перебивается.

### 19.2 Токены вместо литеральных цветов

Компоненты §7.2 больше не пишут `bg-white`/`text-neutral-600` напрямую. Палитра
объявлена CSS-переменными в `web/static/css/input.css` и подключена к Tailwind
в `tailwind.config.js` как семантические цвета:

```
bg  bg-deep  surface  surface-2  plate
ink  ink-muted  ink-faint
hair  hair-strong
accent  accent-fg  gold
success-{bg,ink,hair}  warning-{...}  danger-{...}
```

Значения переменных задаются областью, а не компонентом:

- `:root` - светлые нейтральные значения, ровно те, что были до v6. Поэтому
  админка в браузере не меняется ни на пиксель;
- `.shop` - тёмная ковровая палитра витрины;
- `.admin-mini` - значения выводятся из `--tg-*`.

Одно правило без исключений: цвет в разметке витрины - только через токен.
Литеральный `neutral-*`/`rose-*` в шаблонах витрины - повод отклонить PR.

### 19.3 Палитра витрины

Снята с ковра-первоисточника, значения - `web/static/css/input.css`:

| Токен | Hex | Роль |
|---|---|---|
| `bg` | `#1A0E0D` | фон страницы |
| `bg-deep` | `#120908` | шапка, подвал, ленты |
| `surface` | `#241413` | карточка, панель |
| `surface-2` | `#2E1917` | вложенная поверхность, hover |
| `plate` | `#E9DDC6` | подложка под фото товара |
| `ink` | `#F1E6D2` | основной текст |
| `ink-muted` | `#BC9F86` | второстепенный текст |
| `ink-faint` | `#8A705E` | подписи, счётчики |
| `hair` | `#4A2A26` | границы |
| `accent` / `gold` | `#C8964B` | единственный акцент |
| `accent-fg` | `#1A0E0D` | текст на акценте |
| индиго | `#141B2E` | второй фон ковра, ленты и hero |

Контраст проверен по WCAG AA для текста: `ink` на `bg` ~13:1, `ink-muted` на
`bg` ~5.4:1, `accent-fg` на `accent` ~7:1. Ниже 4.5:1 не опускаемся нигде,
кроме чисто декоративного орнамента.

### 19.4 Орнамент

Орнамент рисуется как SVG, не как растр: он красится `currentColor`, чёткий на
retina, весит килобайты и не требует сетевого запроса, которого не пропустил бы
CSP. Два способа:

- **компонент** `Ornament(kind OrnamentKind)` - добавлен к закрытому списку
  §7.2. `OrnamentBand` - горизонтальная лента-разделитель, `OrnamentRosette` -
  розетка «қошқар мүйіз» как акцент у заголовка. Виды - закрытое перечисление,
  как `Tone` и `ButtonVariant`;
- **фон** - тайл того же мотива, вшитый data-URI в класс `.k-field`
  (`img-src 'self' data:` уже разрешает такой фон, §9.7).

Орнамент всегда декоративен: `aria-hidden="true"`, `focusable="false"`, никакой
информации только через него.

### 19.5 Движение

«Плавность» ограничена тем, что не требует ни JS-обсервера, ни библиотеки:

- переходы `transition-colors`/`opacity`/`transform` 200-500мс на
  `cubic-bezier(.2,.7,.3,1)`;
- появление блоков первого экрана - CSS-анимация `animate-rise` со
  ступенчатой задержкой, без наблюдателей пересечения (Alpine вендорится ядром,
  плагина `intersect` в сборке нет);
- бегущая строка слогана - CSS-анимация `animate-marquee`; дубль строки несёт
  `aria-hidden="true"`, чтобы скринридер прочитал слоган один раз;
- всё перечисленное отключается в `@media (prefers-reduced-motion: reduce)`.

Требование §7.4 «никаких теней-градиентов» смягчается ровно до: тени только как
глубина карточки над фоном, градиент только как затухание ковровой текстуры.
Декоративных подсветок, свечений и цветных теней нет.

### 19.6 Слоган

Каноническая строка бренда, меняется только здесь:

```
qzq.kiim · КЕЛ, ШАЙ ІШЕМІЗ · ЧӘЙ · СӘЙ · ÇAY · ЧАЙ · ШАЙ · СAЙ · ЧАИ · ČAI · ЦӘ
```

Живёт в `templates.Slogan` одной константой и рендерится из неё везде: лента под
шапкой, подвал. Второй копии строки в проекте нет.

### 19.7 Типографика

Заголовки - `font-display`, шрифтовой стек из системных засечных
(`Iowan Old Style`, `Palatino`, `Georgia`, `ui-serif`). Основной текст - прежний
`font-sans`. Веб-шрифт не подключается: `font-src 'self'` (§9.7) запрещает CDN, а
вкладывать бинарный шрифт в репозиторий ради заголовков - плохой размен.
Появится лицензированный шрифт - вендорится в `web/static/fonts` и меняется
только `fontFamily.display`.

---

## 20. Решения v7: примитив итогов

### 20.1 `Totals` в закрытом списке §7.2

Блок «Subtotal / Shipping / Total» стоял тремя дословными копиями разметки: в
корзине (`CartFragment`), на чекауте и на странице статуса заказа. Это ровно то
дублирование, которое §7.2 называет нарушением DRY и поводом отклонить PR.

К закрытому списку §7.2 добавляется:

```templ
templ Totals(subtotal, shipping, total money.Amount)
```

Единственный рендерер итогов в проекте. Смысл не в трёх сэкономленных строках:
пока копий три, строка доставки может появиться на одной странице и потеряться
на другой, а покупатель увидит разные итоги на чекауте и в заказе.

Компонент принимает уже посчитанные суммы и ничего не считает сам: арифметика
остаётся в домене (`money.Amount.Add`, §6.1), инварианты сумм по-прежнему
проверяются property-тестами слайса S2.2, а не через шаблон.
